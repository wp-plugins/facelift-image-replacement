...
var FLIR = {
	...
	,hover: function(e) {
		...
		for(var i=0; i < objs.length; i++) {
			...
			if(!FLIR.isCraptastic) {
				...
				var img = rep_obj.flirImage ? rep_obj : FLIR.getChildren(rep_obj)[0];
				img.style.filter = 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="'+(on?FStyle.generateURL(rep_obj, img.alt):rep_obj.flirOrig)+'", sizingMethod="crop")';
			}
		}
	}
	...
	,replace: function(o, fstyle) {
		...
		if(!o.flirHasHover && FStyle != FStyle.hoverStyle)
			FLIR.addHover(o);
...
FLIRStyle.prototype.generateURL = function(o) { // [, text]
	var enc_text = (arguments[1]?arguments[1]:escape(o.innerHTML)).replace(/&/g, '{amp}'); // opera was messing things up when this was in the return statement
